...
def forgot_password():
    if request.method == "POST":
        ...
        if not user:
            ...
            flash("If that username exists, the password has been reset. You can now log in.", "success")
            return redirect(url_for("auth.login"))

        if not user.is_active:
            flash("This account is disabled. Please contact the administrator.", "danger")
            return render_template("auth/forgot_password.html")
        ...
        log_action(
            ...
            detail=f"Password reset via Forgot Password for username: {username}",
            user_id=user.id
        )
